...
public class OpenDroneIdDataManager {
    ...
    void receiveDataBluetooth(byte[] data, ScanResult result, LogMessageEntry logMessageEntry, String transportType) {
        ...
        if (message == null)
            return;
        receiveData(result.getTimestampNanos(), macAddress, macAddressLong, result.getRssi(), message, logMessageEntry, transportType);

        getReceiverLocation(); // This call ensures the receiver location gets updated at some point in the future
    }
